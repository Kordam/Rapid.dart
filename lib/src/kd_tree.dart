...
// Adapted by Jean Grizet <jean.grizet@gmail.com> for the rapid library
...
class KdTree<T> {
  ...
  KdNode buildTree(List<KdNodeData<T>> data, int depth, KdNode parent) {
    ...
    data.sort((KdNodeData<T> a, KdNodeData<T> b) => (a.vec[dim] - b.vec[dim]).toInt());
  ...
  KdNodeData<T> nearest(Vector4 point) {
    ...
    KdNode<T> findNearest(KdNode<T> node, int depth) {
      ...
      if (dist * dist < point.distanceTo(min.data.vec))
        min = findNearest(far, depth + 1);
      if (point.distanceTo(node.data.vec) < point.distanceTo(min.data.vec))
        min = node;
  ...
  List<KdNodeData<T>> nearestMultiple(Vector4 point, int maxNodes) {
    ...
    void nearestSearch(KdNode<T> node) {
      ...
      linearDistance = linearPoint.distanceTo(node.data.vec);
